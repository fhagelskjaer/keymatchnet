...
# def get_loss(l_pred, cat_pred, feat_pred, label, cat, feat, cat_mask, feat_mask, weight_l, weight_cat, weight_feat, ignore_index=-1):
def get_loss(seg_pred, seg, key_pred, key, weight_key=0.8, ignore_index=-1):

    seg_loss = F.cross_entropy(
        seg_pred, seg, reduction='mean',
        ignore_index=ignore_index)

    key_loss = F.cross_entropy(
        key_pred, key, reduction='mean',
        ignore_index=ignore_index)

    return (1-weight_key) * seg_loss + weight_key * key_loss
    ...
    #per_instance_feat_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=feat_pred, labels=feat), axis=1)
    ##per_instance_seg_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=seg_pred, labels=seg), axis=1)
    #feat_loss = tf.reduce_mean(per_instance_feat_loss)


    feat_softmax = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=feat_pred, labels=feat)
    feat_softmax = tf.where( tf.math.equal(feat_mask, 0), tf.zeros_like(feat_softmax), feat_softmax)     #tf.multiply(feat_softmax, feat_mask)
    
    per_instance_feat_loss = tf.math.divide_no_nan( tf.math.reduce_sum(feat_softmax , axis=1), tf.math.reduce_sum(feat_mask, axis=1) )
    
    #per_instance_seg_loss = per_instance_seg_loss * seg_cat
    #replacement_tensor = tf.zeros_like(per_instance_feat_loss)
    #tf.where(tf.is_nan(per_instance_feat_loss), replacement_tensor, per_instance_feat_loss)
    
    sum_tensor = tf.zeros_like(per_instance_feat_loss) + 1
    sum_tensor = tf.where(tf.math.equal( tf.math.reduce_sum(feat_mask, axis=1), 0), tf.zeros_like(per_instance_feat_loss), sum_tensor)
    
    feat_loss = tf.math.divide_no_nan( tf.math.reduce_sum(per_instance_feat_loss), tf.math.reduce_sum( sum_tensor ) )

    per_instance_feat_pred_res = tf.argmax(feat_pred, 2)
    ...
    #total_loss = weight * seg_cat_loss +  weight * seg_loss + (1 - weight) * label_loss + mat_diff_loss * 1e-3

    #total_loss = weight * cat_loss + weight * feat_loss + (1 - weight) * label_loss + mat_diff_loss * 1e-3
    #total_loss = weight * cat_loss / 3.0 + weight * feat_loss + (1 - weight) * label_loss + mat_diff_loss * 1e-3

    total_loss = weight_cat * cat_loss + weight_feat * feat_loss + weight_l * label_loss + mat_diff_loss * 1e-3


    return total_loss, label_loss, per_instance_label_loss, cat_loss, per_instance_cat_loss, per_instance_cat_pred_res, feat_loss, per_instance_feat_loss, per_instance_feat_pred_res
